...
(function(){
  ...
  function personalizeRoleCard(map){
    ...
    if(!role) return;
    const assigned=map[role]||'nicht gesetzt';
    document.querySelectorAll('[data-assigned-name-for="'+role+'"]').forEach(el=>el.textContent=assigned);
    const pill=document.querySelector('.role-name-pill');
    if(pill) pill.innerHTML=roleSpan(role,displayFor(role,map,false));
  }
