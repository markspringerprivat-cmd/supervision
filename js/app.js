...
/* FINAL PATCH: Navigationsbuttons in globaler Leiste */
(function(){
  function addGlobalNavButtons(){
    if (document.body && document.body.dataset.mode === 'presentation') return;
    const inner = document.querySelector('.local-reset-inner');
    if (!inner) return;
    if (!document.getElementById('globalBackBtn')) {
      const back = document.createElement('button');
      back.type = 'button';
      back.id = 'globalBackBtn';
      back.className = 'secondary small-reset nav-reset-btn';
      ...
      const statusBtn = document.getElementById('globalAdminStatusBtn');
      if (statusBtn && statusBtn.nextSibling) inner.insertBefore(back, statusBtn.nextSibling);
      else inner.insertBefore(back, inner.firstChild);
    }
    if (!document.getElementById('globalHomeBtn')) {
      const home = document.createElement('button');
      home.type = 'button';
      home.id = 'globalHomeBtn';
      home.className = 'secondary small-reset nav-reset-btn';
      ...
      if (back && back.nextSibling) inner.insertBefore(home, back.nextSibling);
      else inner.insertBefore(home, inner.firstChild);
    }
  }
  function install(){
    try { addGlobalNavButtons(); } catch(e) { console.warn(e); }
    setTimeout(addGlobalNavButtons, 120);
    setTimeout(addGlobalNavButtons, 600);
  }
