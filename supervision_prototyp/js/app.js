...
function initResults() {
  ...
  script.src = `${url}?action=list&callback=${cbName}`;
  script.onerror = () => { status.className = "warning"; status.textContent = "Ergebnisse konnten nicht geladen werden."; };
...
function renderResults(rows) {
  ...
  target.innerHTML = rows.reverse().map((row, idx) => {
    const data = row.data || {};
