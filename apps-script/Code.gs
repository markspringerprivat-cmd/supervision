...
function doGet(e) {
  try {
    ...
    if (action === 'savegrouppresentation' || action === 'save_group_presentation' || action === 'updategrouppresentation') return jsonp_(e, { ok:false, type:'saveGroupPresentation', feature:'manometer-game-update-v88', error:'Diese Aktion benötigt POST, damit die Präsentationsdaten vollständig gespeichert werden.' });
    ...
    if (action === 'ping') {
      ...
      return jsonp_(e, { ok: true, message: 'Apps Script läuft.', sheetName: SHEET_NAME, manometer: true, feature: 'manometer-game-update-v88', spreadsheetName: spreadsheetName, deviceRegistrySheet: DEVICE_REGISTRY_SHEET_NAME });
    }
    if (action === 'test') {
...
function manometerFeedbackStatus_(p) {
  ...
  if (!groupId) {
    return { ok:true, type:'manometerFeedbackStatus', feature:'manometer-game-update-v88', checked:false, alreadyVoted:false, needsGroupId:true };
  }
  if (!deviceId) {
    return { ok:true, type:'manometerFeedbackStatus', feature:'manometer-game-update-v88', checked:false, alreadyVoted:false, groupId:groupId };
  }
  ...
  return {
    ...
    feature:'manometer-game-update-v88',
    checked:true,
...
function saveFeedbackData_(data) {
  ...
  data = (data && typeof data === 'object') ? data : {};
  const deviceId = textValue_(data.deviceId || data.device || data.browserId);
  if (!deviceId) return { ok: false, type: 'manometerFeedbackSave', feature: 'manometer-game-update-v88', error: 'Keine Geräte-ID übermittelt.' };
  const groupId = textValue_(data.groupId || data.g || data.groupToken || data.token);
  if (!groupId) return { ok: false, type: 'manometerFeedbackSave', feature: 'manometer-game-update-v88', error: 'Keine Gruppen-ID übermittelt. Bitte den Feedback-QR-Code aus dem Gruppenfortschritt erneut scannen.' };

  const submitted = participationAlreadySubmitted_(deviceId, groupId);
  if (submitted.found) {
    return { ok: false, duplicate: true, type: 'manometerFeedbackSave', feature: 'manometer-game-update-v88', error: 'Von diesem Gerät wurde für diese Feedbackrunde bereits Feedback abgegeben.', rowNumber: submitted.feedbackRow || submitted.rowNumber, participationId: submitted.participationId, source: 'participationSheet' };
  }
  ...
  if (duplicateRow >= 2) {
    ...
    return { ok: false, duplicate: true, type: 'manometerFeedbackSave', feature: 'manometer-game-update-v88', error: 'Von diesem Gerät wurde für diese Feedbackrunde bereits Feedback abgegeben.', rowNumber: duplicateRow, source: 'feedbackSheet' };
  }
  ...
  SpreadsheetApp.flush();
  return { ok: true, type: 'manometerFeedbackSave', feature: 'manometer-game-update-v88', message: 'Manometer-Feedback gespeichert.', rowNumber: savedRow, groupId: groupId };
}
...
function resetManometerDeviceIds_(password) {
  if (String(password || '') !== String(MANOMETER_ADMIN_PASSWORD || '')) {
    return { ok: false, type: 'resetManometerDeviceIds', feature: 'manometer-game-update-v88', error: 'Admin-Passwort fehlt oder ist falsch.' };
  }
...
function deleteManometerFeedbackAll_(password) {
  if (String(password || '') !== String(MANOMETER_ADMIN_PASSWORD || '')) {
    return { ok: false, type: 'deleteManometerFeedbackAll', feature: 'manometer-game-update-v88', error: 'Admin-Passwort fehlt oder ist falsch.' };
  }
...
function registerGroup_(params) {
  ...
  if (!groupId) return { ok: false, type: 'registerGroup', feature: 'manometer-game-update-v88', error: 'Keine Gruppen-ID übermittelt.' };
  const sheet = getGroupRegistrySheet_();
  const now = new Date();
  ...
  if (existing >= 2) {
    ...
    SpreadsheetApp.flush();
    return { ok: true, type: 'registerGroup', feature: 'manometer-game-update-v88', mode: 'updated', groupId: groupId, rowNumber: existing };
  }
  sheet.appendRow(row);
  SpreadsheetApp.flush();
  return { ok: true, type: 'registerGroup', feature: 'manometer-game-update-v88', mode: 'created', groupId: groupId, rowNumber: sheet.getLastRow() };
}
...
function registerDeviceGroup_(params) {
  ...
  const groupId = textValue_(params.groupId || params.g || params.groupToken || params.token);
  if (!deviceId) return { ok: false, type: 'registerDeviceGroup', feature: 'manometer-game-update-v88', error: 'Keine Geräte-ID übermittelt.' };
  if (!groupId) return { ok: false, type: 'registerDeviceGroup', feature: 'manometer-game-update-v88', error: 'Keine Gruppen-ID übermittelt.' };
  ...
  if (existing >= 2) {
    ...
    SpreadsheetApp.flush();
    return { ok: true, type: 'registerDeviceGroup', feature: 'manometer-game-update-v88', mode: 'updated', deviceId: deviceId, groupId: groupId, rowNumber: existing };
  }
  sheet.appendRow(row);
  SpreadsheetApp.flush();
  return { ok: true, type: 'registerDeviceGroup', feature: 'manometer-game-update-v88', mode: 'created', deviceId: deviceId, groupId: groupId, rowNumber: sheet.getLastRow() };
}
...
function resolveDeviceGroup_(params) {
  const deviceId = textValue_(params.deviceId || params.device || params.browserId);
  if (!deviceId) return { ok: false, type: 'resolveDeviceGroup', feature: 'manometer-game-update-v88', error: 'Keine Geräte-ID übermittelt.' };
  const sheet = getDeviceRegistrySheet_();
  const row = findDeviceRegistryRow_(deviceId);
  if (row < 2) return { ok: true, type: 'resolveDeviceGroup', feature: 'manometer-game-update-v88', found: false };
...
function deleteGroupsAll_(password) {
  if (String(password || '') !== String(MANOMETER_ADMIN_PASSWORD || '')) {
    return { ok: false, type: 'deleteGroupsAll', feature: 'manometer-game-update-v88', error: 'Admin-Passwort fehlt oder ist falsch.' };
  }
...
function deleteGroup_(password, groupId) {
  if (String(password || '') !== String(MANOMETER_ADMIN_PASSWORD || '')) {
    return { ok: false, type: 'deleteGroup', feature: 'manometer-game-update-v88', error: 'Admin-Passwort fehlt oder ist falsch.' };
  }
  const gid = textValue_(groupId);
  if (!gid) return { ok: false, type: 'deleteGroup', feature: 'manometer-game-update-v88', error: 'Keine Gruppen-ID übermittelt.' };
...
function deleteMainResultsAll_(password) {
  if (String(password || '') !== String(MANOMETER_ADMIN_PASSWORD || '')) {
    return { ok: false, type: 'deleteMainResultsAll', feature: 'manometer-game-update-v88', error: 'Admin-Passwort fehlt oder ist falsch.' };
  }
...
function createGroupSession_(p) {
  ...
  if (!deviceId) return { ok:false, type:'createGroupSession', feature:'manometer-game-update-v88', error:'Keine Geräte-ID übermittelt.' };
  if (!name) return { ok:false, type:'createGroupSession', feature:'manometer-game-update-v88', error:'Kein Name übermittelt.' };
  ...
  return { ok:true, type:'createGroupSession', feature:'manometer-game-update-v88', groupId:groupId, groupName:groupId, members:members };
}
...
function joinGroupSession_(p) {
  ...
  const name = textValue_(p.name || p.memberName);
  const deviceType = textValue_(p.deviceType || 'Unbekannt');
  if (!groupId) return { ok:false, type:'joinGroupSession', feature:'manometer-game-update-v88', error:'Keine Gruppen-ID übermittelt.' };
  if (!deviceId) return { ok:false, type:'joinGroupSession', feature:'manometer-game-update-v88', error:'Keine Geräte-ID übermittelt.' };
  if (!name) return { ok:false, type:'joinGroupSession', feature:'manometer-game-update-v88', error:'Kein Name übermittelt.' };
  ...
  if (existingMemberRow < 2 && existingMembers.length >= 5) {
    return { ok:false, type:'joinGroupSession', feature:'manometer-game-update-v88', error:'Diese Gruppe ist bereits voll. Maximal 5 Mitglieder sind möglich.' };
  }
  ...
  return { ok:true, type:'joinGroupSession', feature:'manometer-game-update-v88', groupId:groupId, groupName:groupName, members:members, rolesCleared:true };
}
...
function listGroupMembers_(p) {
  const groupId = textValue_(p.groupId || p.g);
  if (!groupId) return { ok:false, type:'listGroupMembers', feature:'manometer-game-update-v88', error:'Keine Gruppen-ID übermittelt.' };
  const members = membersForGroup_(groupId);
  const groupName = sessionGroupNameFromMembers_(members) || groupId;
  return { ok:true, type:'listGroupMembers', feature:'manometer-game-update-v88', groupId:groupId, groupName:groupName, members:members };
}
...
function removeGroupMember_(p) {
  ...
  if (!groupId) return { ok:false, type:'removeGroupMember', feature:'manometer-game-update-v88', error:'Keine Gruppen-ID übermittelt.' };
  if (!deviceIdToRemove && removeRowNumber < 2) return { ok:false, type:'removeGroupMember', feature:'manometer-game-update-v88', error:'Kein Mitglied zum Entfernen übermittelt.' };

  const requesterRow = requesterDeviceId ? findGroupMemberRow_(groupId, requesterDeviceId) : 0;
  if (requesterRow < 2) return { ok:false, type:'removeGroupMember', feature:'manometer-game-update-v88', error:'Nur der Gruppenanführer kann Mitglieder entfernen.' };
  ...
  if (String(requester[6]).toLowerCase() !== 'true') {
    return { ok:false, type:'removeGroupMember', feature:'manometer-game-update-v88', error:'Nur der Gruppenanführer kann Mitglieder entfernen.' };
  }
  ...
  if (removeRow < 2) return { ok:false, type:'removeGroupMember', feature:'manometer-game-update-v88', error:'Dieses Mitglied wurde nicht gefunden.' };
  ...
  if (String(row[6]).toLowerCase() === 'true') return { ok:false, type:'removeGroupMember', feature:'manometer-game-update-v88', error:'Der Gruppenanführer kann nicht aus der Gruppe entfernt werden.' };
  ...
  SpreadsheetApp.flush();
  return { ok:true, type:'removeGroupMember', feature:'manometer-game-update-v88', groupId:groupId, removedDeviceId:actualDeviceIdToRemove, deletedDeviceData:deletedDeviceData, members:members };
}
...
function assignRolesToMembers_(p) {
  ...
  try {
    const groupId = textValue_(p.groupId || p.g);
    if (!groupId) return { ok:false, type:'assignRolesToMembers', feature:'manometer-game-update-v88', error:'Keine Gruppen-ID übermittelt.' };

    let members = membersForGroup_(groupId);
    if (members.length < 4) return { ok:false, type:'assignRolesToMembers', feature:'manometer-game-update-v88', error:'Mindestens 4 Gruppenmitglieder erforderlich.' };
    if (members.length > 5) return { ok:false, type:'assignRolesToMembers', feature:'manometer-game-update-v88', error:'Maximal 5 Gruppenmitglieder sind möglich.' };
...
function resolveAssignedRoleForDevice_(p) {
  const groupId = textValue_(p.groupId || p.g);
  const deviceId = textValue_(p.deviceId || p.device || p.browserId);
  if (!groupId || !deviceId) return { ok:false, type:'resolveAssignedRoleForDevice', feature:'manometer-game-update-v88', error:'Gruppen-ID oder Geräte-ID fehlt.' };
  const rowNo = findGroupMemberRow_(groupId, deviceId);
  if (rowNo < 2) return { ok:true, type:'resolveAssignedRoleForDevice', feature:'manometer-game-update-v88', found:false };
  const row = getGroupMemberSheet_().getRange(rowNo, 1, 1, GROUP_MEMBER_HEADERS.length).getValues()[0];
  return { ok:true, type:'resolveAssignedRoleForDevice', feature:'manometer-game-update-v88', found:true, groupId:row[1]||'', deviceId:row[2]||'', name:row[3]||'', deviceType:row[4]||'', role:row[5]||'', isPrimary:String(row[6]).toLowerCase()==='true' };
}
...
function listGroups_(params) {
  ...
  return { ok: true, type: 'listGroups', feature: 'manometer-game-update-v88', groups: groups, total: groups.length };
}
...
function checkGroupExists_(p) {
  ...
  const deviceId = textValue_(p.deviceId || p.device || p.browserId);
  if (!groupId && !deviceId) {
    return { ok: true, type: 'checkGroupExists', feature: 'manometer-game-update-v88', checked: false, exists: true };
  }
  ...
  if (!resolvedGroupId) {
    return { ok: true, type: 'checkGroupExists', feature: 'manometer-game-update-v88', checked: true, exists: false, groupId: '', reason: 'Keine gültige Gruppen-ID gefunden.' };
  }
...
function saveGroupPresentation_(body) {
  ...
  if (!groupId) return { ok:false, type:'saveGroupPresentation', feature:'manometer-game-update-v88', error:'Keine Gruppen-ID übermittelt.' };
  ...
  if (rowNo < 2) return { ok:false, type:'saveGroupPresentation', feature:'manometer-game-update-v88', error:'Kein Gruppenergebnis für diese Gruppen-ID gefunden.' };
  ...
  return { ok:true, type:'saveGroupPresentation', feature:'manometer-game-update-v88', message:'Gruppenpräsentation gespeichert.', groupId:groupId, rowNumber:rowNo, editorSaveId:normalized.editorSaveId };
}
